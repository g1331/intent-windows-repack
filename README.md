...
`windows-latest` runner 自带 VS 2022 C++ 工具链 / Node / Python / 7-Zip,正好满足全部编译需求。支持两种运行方式:

- **定时自动发版**:工作流每天检查更新源,发现未发布过的新版本就自动重打包,并发布到对应 `v<版本>` 的 Release,无需人工操作。
- **手动触发**:Actions 页 → **Build Intent for Windows** → **Run workflow**。
  - `dmg_url`:可选。直接给一个 `.dmg` 直链;**留空则自动获取最新稳定版**。
...
成品到 **Artifacts** 下载;自动获取场景同时发布到 **Releases**。
